# -*- coding: utf-8 -*-
"""
مزامنة بيانات الأغاني من Google Sheets إلى Supabase
...
def sync_songs():
    """مزامنة الأغاني (إضافة الجديد فقط)"""
    logger.info("="*60)
    logger.info("🔄 بدء مزامنة الأغاني...")
    
    if not SUPABASE_URL or not SUPABASE_KEY:
        logger.error("SUPABASE_URL أو SUPABASE_KEY غير مضبوط")
        return False
    
    # جلب الأغاني من Google Sheets
    songs = get_all_songs_from_sheets()
    if not songs:
        logger.error("❌ فشل جلب البيانات من Google Sheets")
        return False
    ...
    try:
        ...
        existing_names = {song['name'] for song in response.data}
    ...
    for song in songs:
        if song['name'] not in existing_names:
            try:
                ...
                new_count += 1
                logger.info(f"✅ تم إضافة: {song['name']}")
            except Exception as e:
                logger.error(f"❌ فشل إضافة {song['name']}: {e}")
    
    logger.info(f"🎉 تمت المزامنة بنجاح! تم إضافة {new_count} أغنية جديدة")
    logger.info("="*60)
    return True
